...
const ForceSimulationDeckGL = ({
  ...
  // Scaling functions to map x, y to longitude and latitude
  const xScale = d3.scaleLinear().domain([0, width]).range([-180, 180]);

  const yScale = d3.scaleLinear().domain([0, height]).range([-85, 85]);
  useEffect(() => {
    // Initialize nodes
    const initialNodes = [];
    for (let i = 0; i < 50; i++) {
    ...
    const animate = (time) => {
      ...
      // Map x, y to longitude and latitude
      simulationNodes.forEach((node) => {
        node.longitude = xScale(node.x);
        node.latitude = yScale(node.y);
      });
