export default function VisibleTextBlock({
  ...
  const textBlocks = [
    ...
    "The size of each circle on the map is proportional to the emissions that would be reduced if that landfill were capped. Despite being very large, many waste sites already have good practices in place that make for a low emissions intensity, or lorem ipsum. For example, landfill X is blah blah but capping it would only  lorem ipsum. Now, we can question our second assumption: The most-emitting landfills may not actually provide much return on our investment if covering them won’t reduce emissions much.",
    "In the bar chart below, we sort all landfills in the world by the emissions reductions that would actually result in capping them. The chart shows the top 200 such landfills. In our current scenario, the 100 landfills that we’ve capped are sprinkled through the chart – many landfills in gray have huge potential, but weren’t capped in this scenario. Ultimately, Scenario 1 reduces emissions by 1.6 Gt1.3 million Mt, or 4.3% of all landfill emissions.",
